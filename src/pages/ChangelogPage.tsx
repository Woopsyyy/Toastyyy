...
const changelogReleases = [
  ...
  },
  {
    ...
    description:
      "Consolidating the gourmet lab into a continuous single-page scrolling experience, removing favorites database dependencies, and integrating the visual customizer directly beside the recipes grid.",
    features: [
      {
        icon: <Zap className="w-4 h-4 text-accent" />,
        label:
          "Consolidated Builder and Docs into a high-performance single-page scroll layout",
      },
      {
        icon: <Sparkles className="w-4 h-4 text-accent" />,
        label:
          "Embedded the Visual Customizer directly side-by-side with preset cards grid",
      },
      {
        icon: <ShieldCheck className="w-4 h-4 text-accent" />,
        label:
          "Removed Favorites feature to align with database-free architecture",
      },
    ],
    fixes: [
      "Corrected responsive layout overlap on viewports under 1024px",
      "Fixed anchor jump alignment shifting",
    ],
  },
  {
    version: "1.1.0",
    ...
    description:
      "Adding premium loading and promise toast variants to the global notification queue, complete with custom animations.",
    features: [
      {
        icon: <Zap className="w-4 h-4 text-accent" />,
        label:
          'New "loading" toast variant with high-fidelity animated spinner',
      },
      {
        icon: <Sparkles className="w-4 h-4 text-accent" />,
        label: 'New "promise" toast variant with custom pulse motion',
      },
      {
        icon: <ShieldCheck className="w-4 h-4 text-accent" />,
        label: "Bumped NPM library package target to v1.1.0",
      },
    ],
  ...
  },
  {
    ...
    description:
      "A complete aesthetic redesign. Introducing the hand-crafted interactive Toast Mascot, spring physics, and gourmet UI presets.",
    features: [
      {
        icon: <Zap className="w-4 h-4 text-accent" />,
        label:
          "Spring-driven active Mascot (with gaze tracking and hover squash)",
      },
      {
        icon: <Sparkles className="w-4 h-4 text-accent" />,
        label: "Complete creative lab visual overhaul (soft cream-glass theme)",
      },
      {
        icon: <ShieldCheck className="w-4 h-4 text-accent" />,
        label: "Hardened monorepo security setup and zero-comment compliance",
      },
    ],
    fixes: [
      "Resolved fluid alignment shifts on high DPI viewports",
      "Removed legacy style class clutter",
    ],
  },
  {
    version: "0.9.4",
    ...
    description:
      "Tuned notification boundary collision parameters for seamless visual stacking and overlays.",
    features: [
      {
        icon: <Tag className="w-4 h-4 text-accent" />,
        label: "Gourmet flavor preset variables (Strawberry Jam, Burnt Garlic)",
      },
    ],
  ...
  },
  {
    ...
    description:
      "Initial architectural framework introducing the centralized ToastProvider context queue.",
    features: [
      {
        icon: <GitBranch className="w-4 h-4 text-accent" />,
        label: "Core global useToasts queue engine",
      },
    ],
    fixes: [],
  },
    ...
    <div className="container-tight py-16 px-6 relative z-10">
      <div className="max-w-2xl mb-20 text-center mx-auto">
        ...
        >
          ...
          <span className="text-[10px] font-extrabold text-accent-2 tracking-widest uppercase">
            Changelog Feed
          </span>
        </motion.div>

        <h1 className="text-4xl lg:text-5xl font-extrabold tracking-tight text-text mb-4">
          Latest Recipes
        </h1>
        <p className="text-text-2 text-sm md:text-base leading-relaxed max-w-xl mx-auto">
          Trace the evolutionary path of the Toastyyy ecosystem. Discover new
          features, optimization notes, and patch logs.
        </p>
      </div>
          ...
          {changelogReleases.map((release, i) => (
            ...
            >
              <div className="absolute left-4 lg:left-1/2 top-4 lg:-translate-x-1/2 flex flex-col items-center z-10 bg-bg py-2">
                <div
                  className={`w-3.5 h-3.5 rounded-full border-2 border-white ${i === 0 ? "bg-accent shadow-md" : "bg-text-3"}`}
                />
              ...
              <div className="w-full lg:w-1/2 pl-12 lg:pl-0 flex flex-col justify-center">
                <div
                  className={`flex flex-col ${i % 2 === 0 ? "lg:items-end lg:text-right" : "lg:items-start lg:text-left"}`}
                >
                  <div className="flex items-center gap-2.5 mb-3">
                    ...
                    <span className="text-xs font-extrabold text-text-3 font-mono">
                      {release.version}
                    </span>
                  </div>

                  <h2 className="text-xl md:text-2xl font-extrabold mb-3 tracking-tight text-text">
                    {release.title}
                  </h2>
                  ...
                  <span className="text-[10px] font-extrabold text-text-3 mt-1 block select-none lg:hidden">
                    {release.date}
                  </span>
                </div>
              </div>
              ...
              <div className="w-full lg:w-1/2 pl-12 lg:pl-0">
                <div className="glass rounded-[28px] p-6 border-accent/10 relative overflow-hidden shadow-md">
                  <div className="relative z-10">
                    <h4 className="text-[9px] font-extrabold text-text-3 uppercase tracking-widest mb-4">
                      Gourmet Changes
                    </h4>
                    ...
                    <ul className="space-y-3.5 mb-6">
                      {release.features.map((feature, j) => (
                        <li key={j} className="flex gap-3">
                          ...
                          <span className="text-xs text-text-2 leading-relaxed font-semibold">
                            {feature.label}
                          </span>
                        </li>
                      ))}
                      ...
                      <div className="pt-5 border-t border-border-strong">
                        ...
                        <ul className="space-y-2">
                          {release.fixes.map((fix, j) => (
                            <li
                              key={j}
                              className="text-xs text-text-2 flex items-start gap-2 font-semibold"
                            >
          ...
          <div className="flex gap-6 items-center">
            ...
            <div>
              ...
              <p className="text-text-2 text-xs md:text-sm leading-relaxed max-w-sm">
                Get monthly recipes, gooey presets, performance patches, and
                updates delivered straight to your workbench.
              </p>
            </div>
          </div>
