  ...
  const firePresetToast = (preset: string) => {
    switch (preset) {
      ...
      case 'promise-success-pill': {
        const id = addToast({ type: 'loading', title: 'Bundling changes...', showProgress: true })
        setTimeout(() => {
          updateToast(id, { type: 'success', title: 'Changes saved', description: 'All changes synced successfully.', showDescription: true })
        }, 2000)
        break
      }
      case 'promise-error-pill': {
        const id = addToast({ type: 'loading', title: 'Baking sourdough...', showProgress: true })
        setTimeout(() => {
          updateToast(id, { type: 'error', title: 'Something went wrong', description: 'Chamber temperature dropped.', showDescription: true, showAction: true, actionText: 'Retry' })
        }, 2000)
        break
      }
      case 'promise-success-expanded': {
        const id = addToast({ type: 'loading', title: 'Compiling gourmet core...', showProgress: true })
        setTimeout(() => {
          updateToast(id, { type: 'success', title: 'Compilation Complete', description: 'Created 4 bundles in 120ms.', showDescription: true })
        }, 2000)
        break
      }
    ...
    <div className="container-wide relative z-10 min-h-screen pt-24" id="docs">
      ...
      <main className="w-full py-4 space-y-20 pb-32">
        <header className="mb-12">
          <h1 className="text-4xl lg:text-5xl font-black tracking-tight text-text leading-none mb-3">Documentation</h1>
        ...
        <section id="doc-quick-start" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-extrabold tracking-tight text-text">Quick Start</h2>
          </div>
          ...
          <div className="bg-white border border-border-strong rounded-2xl overflow-hidden shadow-sm flex flex-col">
            <div className="px-4 py-2.5 bg-surface-2 border-b border-border-strong flex justify-between items-center select-none">
              <span className="text-[9px] font-extrabold text-text-3 tracking-wider uppercase">App.tsx</span>
              <button 
                onClick={() => handleCopyCode('quickStart', codeSnippets.quickStart)}
                className="p-1.5 rounded-lg hover:bg-border-strong text-text-3 hover:text-accent transition-all"
              >
                {copiedId === 'quickStart' ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
        ...
        <section id="doc-shadcn" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-extrabold tracking-tight text-text">shadcn/ui</h2>
          </div>
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
          ...
          <div className="bg-white border border-border-strong rounded-2xl p-4 flex items-center justify-between font-mono text-[11.5px] text-text select-all shadow-sm">
            <div className="flex items-center gap-2 overflow-x-auto pr-4 scrollbar-none">
              ...
              <span className="whitespace-nowrap text-text-2 font-semibold">{codeSnippets.shadcnCli}</span>
            </div>
            <button 
              onClick={() => handleCopyCode('shadcnCli', codeSnippets.shadcnCli)}
              className="p-1.5 rounded-lg hover:bg-border-strong text-text-3 hover:text-accent transition-all flex-shrink-0"
            >
              {copiedId === 'shadcnCli' ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5" />}
            </button>
          </div>
          ...
          <div className="bg-white border border-border-strong rounded-2xl overflow-hidden shadow-sm flex flex-col">
            <div className="px-4 py-2.5 bg-surface-2 border-b border-border-strong flex justify-between items-center select-none">
              <span className="text-[9px] font-extrabold text-text-3 tracking-wider uppercase">layout.tsx</span>
              <button 
                onClick={() => handleCopyCode('shadcnImport', codeSnippets.shadcnImport)}
                className="p-1.5 rounded-lg hover:bg-border-strong text-text-3 hover:text-accent transition-all"
              >
                {copiedId === 'shadcnImport' ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
            <pre className="p-5 font-mono text-[11.5px] text-text-2 overflow-x-auto leading-relaxed select-all bg-white">
        ...
        <section id="doc-types" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-extrabold tracking-tight text-text">Toast Types</h2>
          </div>
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
            Centralized notification presets designed to cover the core semantic states.
          </p>

          <div className="bg-white border border-border-strong rounded-2xl overflow-hidden shadow-sm flex flex-col">
            <div className="px-4 py-2.5 bg-surface-2 border-b border-border-strong flex justify-between items-center select-none">
              <span className="text-[9px] font-extrabold text-text-3 tracking-wider uppercase">API Methods</span>
              <button 
                onClick={() => handleCopyCode('types', codeSnippets.types)}
                className="p-1.5 rounded-lg hover:bg-border-strong text-text-3 hover:text-accent transition-all"
              >
                {copiedId === 'types' ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
            <pre className="p-5 font-mono text-[11.5px] text-text-2 overflow-x-auto leading-relaxed select-all bg-white">
        ...
        <section id="doc-description" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-extrabold tracking-tight text-text">Description</h2>
          </div>
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
          ...
          <div className="bg-white border border-border-strong rounded-2xl overflow-hidden shadow-sm flex flex-col">
            <div className="px-4 py-2.5 bg-surface-2 border-b border-border-strong flex justify-between items-center select-none">
              <span className="text-[9px] font-extrabold text-text-3 tracking-wider uppercase">Example.ts</span>
              <button 
                onClick={() => handleCopyCode('description', codeSnippets.description)}
                className="p-1.5 rounded-lg hover:bg-border-strong text-text-3 hover:text-accent transition-all"
              >
                {copiedId === 'description' ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
            <pre className="p-5 font-mono text-[11.5px] text-text-2 overflow-x-auto leading-relaxed select-all bg-white">
        ...
        <section id="doc-action-button" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-extrabold tracking-tight text-text">Action Button</h2>
          </div>
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
          ...
          <div className="bg-white border border-border-strong rounded-2xl overflow-hidden shadow-sm flex flex-col">
            <div className="px-4 py-2.5 bg-surface-2 border-b border-border-strong flex justify-between items-center select-none">
              <span className="text-[9px] font-extrabold text-text-3 tracking-wider uppercase">InteractiveAction.ts</span>
              <button 
                onClick={() => handleCopyCode('action', codeSnippets.action)}
                className="p-1.5 rounded-lg hover:bg-border-strong text-text-3 hover:text-accent transition-all"
              >
                {copiedId === 'action' ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
            <pre className="p-5 font-mono text-[11.5px] text-text-2 overflow-x-auto leading-relaxed select-all bg-white">
        ...
        {/* 06 Promise Toasts */}
        <section id="doc-promise-toasts" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            <span className="text-sm font-mono font-black text-accent">06</span>
            <h2 className="text-xl font-extrabold tracking-tight text-text">Promise Toasts</h2>
          </div>
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
          ...
          <div className="bg-white border border-border-strong rounded-2xl overflow-hidden shadow-sm flex flex-col">
            <div className="px-4 py-2.5 bg-surface-2 border-b border-border-strong flex justify-between items-center select-none">
              <span className="text-[9px] font-extrabold text-text-3 tracking-wider uppercase">PromiseLifecycle.ts</span>
              <button 
                onClick={() => handleCopyCode('promise', codeSnippets.promise)}
                className="p-1.5 rounded-lg hover:bg-border-strong text-text-3 hover:text-accent transition-all"
              >
                {copiedId === 'promise' ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
            <pre className="p-5 font-mono text-[11.5px] text-text-2 overflow-x-auto leading-relaxed select-all bg-white">
        ...
        <section id="doc-timings" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-extrabold tracking-tight text-text">Timings</h2>
          </div>
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-border">
                <tr className="hover:bg-surface-2/40 transition-colors">
                  <td className="py-3.5 px-4 font-mono font-bold text-accent">displayDuration</td>
                  <td className="py-3.5 px-4 text-text-2">number</td>
                  <td className="py-3.5 px-4 text-text-2">4000</td>
                  <td className="py-3.5 px-4 text-text-2">Milliseconds toast stays visible</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <section id="doc-toaster-props" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-extrabold tracking-tight text-text">Toaster Props</h2>
          </div>
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
          ...
          <div className="bg-white border border-border-strong rounded-2xl overflow-hidden shadow-sm flex flex-col max-w-sm">
            <div className="px-4 py-2 bg-surface-2 border-b border-border-strong flex justify-between items-center select-none">
              <span className="text-[9px] font-extrabold text-text-3 tracking-wider uppercase">JSX element</span>
            </div>
          ...
          <div className="glass rounded-[24px] border border-border-strong overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-border">
                  {[
                    { p: 'position', t: 'string', d: "'bottom-right'", ds: '6 positions: top-left, top-center, top-right, bottom-left, bottom-center, bottom-right' },
                    { p: 'duration', t: 'number', d: '—', ds: 'Default display duration (ms)' },
                    { p: 'gap', t: 'number', d: '14', ds: 'Gap between stacked toasts' },
                    { p: 'offset', t: 'number | string', d: "'24px'", ds: 'Distance from screen edge' },
                    { p: 'theme', t: "'light' | 'dark'", d: "'light'", ds: 'Color theme' },
                    { p: 'spring', t: 'boolean', d: 'true', ds: 'Enable spring/bounce animations globally' },
                    { p: 'bounce', t: 'number', d: '0.4', ds: 'Spring intensity: 0.05 (subtle) to 0.8 (dramatic)' },
                    { p: 'closeOnEscape', t: 'boolean', d: 'true', ds: 'Dismiss most recent toast on Escape key press' },
                    { p: 'showProgress', t: 'boolean', d: 'false', ds: 'Show countdown progress bar on all toasts' },
                    { p: 'maxQueue', t: 'number', d: 'Infinity', ds: 'Maximum number of toasts in the waiting queue' },
                    { p: 'queueOverflow', t: "'drop-oldest' | 'drop-newest'", d: "'drop-oldest'", ds: 'Behavior when queue exceeds maxQueue' },
                    { p: 'swipeToDismiss', t: 'boolean', d: 'true', ds: 'Enable swipe-to-dismiss touch gestures on mobile' },
                    { p: 'preset', t: 'AnimationPresetName', d: '—', ds: 'Named animation preset (smooth, bouncy, subtle, snappy)' },
                  ].map((row) => (
                    <tr key={row.p} className="hover:bg-surface-2/40 transition-colors">
                      <td className="py-3 px-4 font-mono font-bold text-accent">{row.p}</td>
                      <td className="py-3 px-4 text-text-2">{row.t}</td>
                      <td className="py-3 px-4 text-text-2 font-mono text-[10px]">{row.d}</td>
                      <td className="py-3 px-4 text-text-2 leading-relaxed">{row.ds}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <section id="doc-options" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-extrabold tracking-tight text-text">Options</h2>
          </div>
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
            Per-toast parameters passed in the option argument to configure individual variants.
          </p>
          ...
          <div className="glass rounded-[24px] border border-border-strong overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-border">
                  {[
                    { o: 'description', t: 'ReactNode', d: 'Body content (string or component)' },
                    { o: 'action', t: 'GooeyToastAction', d: 'Action button config' },
                    { o: 'icon', t: 'ReactNode', d: 'Custom icon override' },
                    { o: 'duration', t: 'number', d: 'Display duration in ms' },
                    { o: 'id', t: 'string | number', d: 'Unique toast identifier' },
                    { o: 'classNames', t: 'GooeyToastClassNames', d: 'CSS class overrides' },
                    { o: 'fillColor', t: 'string', d: 'Background color of the blob' },
                    { o: 'borderColor', t: 'string', d: 'Border color of the blob' },
                    { o: 'borderWidth', t: 'number', d: 'Border width in px (default 1.5)' },
                    { o: 'timing', t: 'GooeyToastTimings', d: 'Animation timing overrides' },
                    { o: 'spring', t: 'boolean', d: 'Enable spring/bounce animations (default true)' },
                    { o: 'bounce', t: 'number', d: 'Spring intensity: 0.05 (subtle) to 0.8 (dramatic), default 0.4' },
                    { o: 'showProgress', t: 'boolean', d: 'Show countdown progress bar on this toast' },
                    { o: 'preset', t: 'AnimationPresetName', d: 'Named animation preset (smooth, bouncy, subtle, snappy)' },
                    { o: 'onDismiss', t: '(id: string | number) => void', d: 'Callback fired when toast is dismissed (any reason)' },
                    { o: 'onAutoClose', t: '(id: string | number) => void', d: 'Callback fired only when toast auto-closes (timer)' },
                  ].map((row) => (
                    <tr key={row.o} className="hover:bg-surface-2/40 transition-colors">
                      <td className="py-3 px-4 font-mono font-bold text-accent">{row.o}</td>
                      <td className="py-3 px-4 text-text-2 font-mono text-[10px]">{row.t}</td>
                      <td className="py-3 px-4 text-text-2 leading-relaxed">{row.d}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <section id="doc-methods" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-extrabold tracking-tight text-text">Methods</h2>
          </div>
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
            Beyond the basic gooeyToast() and type methods, the following methods are available for managing toasts programmatically.
          </p>
          ...
          <div className="glass rounded-[24px] border border-border-strong overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-border">
                  <tr className="hover:bg-surface-2/40 transition-colors">
                    <td className="py-3 px-4 font-mono font-bold text-accent">gooeyToast.dismiss</td>
                    <td className="py-3 px-4 text-text-2 font-mono text-[10px]">(id?: string | number) =&gt; void</td>
                    <td className="py-3 px-4 text-text-2 leading-relaxed">Dismiss a specific toast by ID, or all toasts if no ID</td>
                  </tr>
                  <tr className="hover:bg-surface-2/40 transition-colors">
                    <td className="py-3 px-4 font-mono font-bold text-accent">gooeyToast.dismiss</td>
                    <td className="py-3 px-4 text-text-2 font-mono text-[10px]">(filter: DismissFilter) =&gt; void</td>
                    <td className="py-3 px-4 text-text-2 leading-relaxed">Dismiss all toasts matching a type filter</td>
                  </tr>
                  <tr className="hover:bg-surface-2/40 transition-colors">
                    <td className="py-3 px-4 font-mono font-bold text-accent">gooeyToast.update</td>
                    <td className="py-3 px-4 text-text-2 font-mono text-[10px]">(id, options: GooeyToastUpdateOptions) =&gt; void</td>
                    <td className="py-3 px-4 text-text-2 leading-relaxed">Update an active toast's title, description, type, or action in place</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="bg-white border border-border-strong rounded-2xl overflow-hidden shadow-sm flex flex-col">
            <div className="px-4 py-2.5 bg-surface-2 border-b border-border-strong flex justify-between items-center select-none">
              <span className="text-[9px] font-extrabold text-text-3 tracking-wider uppercase">DismissFilter Interface</span>
              <button 
                onClick={() => handleCopyCode('dismissFilter', codeSnippets.dismissFilter)}
                className="p-1.5 rounded-lg hover:bg-border-strong text-text-3 hover:text-accent transition-all"
              >
                {copiedId === 'dismissFilter' ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
            <pre className="p-5 font-mono text-[11.5px] text-text-2 overflow-x-auto leading-relaxed select-all bg-white">
          ...
          <div className="bg-white border border-border-strong rounded-2xl overflow-hidden shadow-sm flex flex-col">
            <div className="px-4 py-2.5 bg-surface-2 border-b border-border-strong flex justify-between items-center select-none">
              <span className="text-[9px] font-extrabold text-text-3 tracking-wider uppercase">GooeyToastUpdateOptions Interface</span>
              <button 
                onClick={() => handleCopyCode('update', codeSnippets.update)}
                className="p-1.5 rounded-lg hover:bg-border-strong text-text-3 hover:text-accent transition-all"
              >
                {copiedId === 'update' ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
            <pre className="p-5 font-mono text-[11.5px] text-text-2 overflow-x-auto leading-relaxed select-all bg-white">
        ...
        {/* 11 Custom Styling */}
        <section id="doc-custom-styling" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            <span className="text-sm font-mono font-black text-accent">11</span>
            <h2 className="text-xl font-extrabold tracking-tight text-text">Custom Styling</h2>
          </div>
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
          ...
          <div className="bg-white border border-border-strong rounded-2xl overflow-hidden shadow-sm flex flex-col">
            <div className="px-4 py-2.5 bg-surface-2 border-b border-border-strong flex justify-between items-center select-none">
              <span className="text-[9px] font-extrabold text-text-3 tracking-wider uppercase">CustomStyles.ts</span>
              <button 
                onClick={() => handleCopyCode('styling', codeSnippets.styling)}
                className="p-1.5 rounded-lg hover:bg-border-strong text-text-3 hover:text-accent transition-all"
              >
                {copiedId === 'styling' ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
            <pre className="p-5 font-mono text-[11.5px] text-text-2 overflow-x-auto leading-relaxed select-all bg-white">
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-border">
                {[
                  ...
                  <tr key={row.k} className="hover:bg-surface-2/40 transition-colors">
                    <td className="py-3 px-4 font-mono font-bold text-accent">{row.k}</td>
                    <td className="py-3 px-4 text-text-2">{row.t}</td>
                  </tr>
                ))}
              </tbody>
        ...
        {/* 12 Spring Animation */}
        <section id="doc-spring-animation" className="space-y-6 pt-6 scroll-mt-28">
          <div className="flex items-center gap-3">
            <span className="text-sm font-mono font-black text-accent">12</span>
            <h2 className="text-xl font-extrabold tracking-tight text-text">Spring Animation</h2>
          </div>
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
            Disable the spring/bounce effect for a cleaner, more subtle animation style. Set per-toast or globally on the Toaster.
          </p>

          <div className="bg-white border border-border-strong rounded-2xl overflow-hidden shadow-sm flex flex-col">
            <div className="px-4 py-2.5 bg-surface-2 border-b border-border-strong flex justify-between items-center select-none">
              <span className="text-[9px] font-extrabold text-text-3 tracking-wider uppercase">SpringConfig.ts</span>
              <button 
                onClick={() => handleCopyCode('spring', codeSnippets.spring)}
                className="p-1.5 rounded-lg hover:bg-border-strong text-text-3 hover:text-accent transition-all"
              >
                {copiedId === 'spring' ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
            <pre className="p-5 font-mono text-[11.5px] text-text-2 overflow-x-auto leading-relaxed select-all bg-white">
          ...
          <p className="text-text-2 text-xs md:text-sm leading-relaxed">
            When spring is false, all spring-based animations (landing squish, blob morph, pill resize, header squish) use smooth ease-in-out curves instead. Error shake still works regardless. Per-toast values override the global setting.
          </p>
